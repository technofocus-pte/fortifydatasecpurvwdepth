...
1. Create a sensitive-information policy
1. Create an inappropriate-text policy
1. Create a conflict-of-interest policy
1. Create a notice template
1. Investigate and remediate an alert
...
1. Install the Exchange Online module and connect. When the sign-in window appears, sign in as **Megan Bowen**:
...
1. Create a moderated, closed distribution group to represent the monitored population:
...
1. Tag the group with a custom attribute so monitored users are easy to track:

    `Set-DistributionGroup -Identity "Contoso Monitored Communications" -CustomAttribute1 "MonitoredCommunication"`

1. Add the research and commercial users whose communications are most sensitive. Replace the identities with real users in your tenant as needed:
...
1. Confirm the group and its members:

    `Get-DistributionGroupMember -Identity "Contoso Monitored Communications" | Format-Table DisplayName, PrimarySmtpAddress`

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image1.png)
...
1. Select **Policies**, then select **+ Create policy** > **Detect sensitive info** (the sensitive-information template).

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image2.png)

1. On the policy page, confirm or set the **Policy name** to `Detect research and trial data in messages`. (Policy names can't be changed after creation.)

1. In the **Users or groups in scope** field, select the **Contoso Monitored Communications** group from Task 1.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image3.png)

1. In the **Reviewers** field, add **Megan Bowen**. (Reviewers added here are the people you can escalate alerts to later.)

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image4.png)

1. On the conditions step, under **Content contains sensitive info types**, select **Add** > **Sensitive info types**, search for `Contoso`, select **Contoso Compound ID** and **Contoso Clinical Trial Subjects** (from Lab 2), then select **Add**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image5.png)

1. Select **Customize policy**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image6.png)

1. Select **Next** until the **Choose conditions and review percentage** page. Enable **Use OCR to extract text from images** so sensitive content in screenshots is also detected, set the **Review percentage** to `100%`, then continue.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image7.png)

1. Review the settings and select **Create policy**, then select **Done**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image8.png)

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image9.png)
...
1. In the Microsoft Purview portal, on the **Communication Compliance** > **Policies** page, select **+ Create policy** > **Detect inappropriate text**.
    ...
    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image10.png)

1. Confirm or set the **Policy name** to `Detect inappropriate text`.

1. In the **Users or groups in scope** field, select the **Contoso Monitored Communications** group.

1. In the **Reviewers** field, add **Megan Bowen**.

1. Accept the template's built-in conditions, which apply Microsoft's threat, harassment, discrimination, and profanity classifiers. Set the **Review percentage** as desired (for example, 100% for the lab), then continue.

1. Review the settings and select **Create policy**, then select **Done**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image11.png)

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image12.png)
...
1. In the Microsoft Purview portal, on the **Communication Compliance** > **Policies** page, select **+ Create policy** > **Detect conflict of interest**.
    ...
    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image13.png)

1. Confirm or set the **Policy name** to `Falcon deal team conflict of interest`.

1. The conflict-of-interest template asks for **two** scoped groups (or two users) to detect internal communications between them. For the first group, select the **Project Falcon** group; for the second, select the **Sales** group (or a Sales/commercial group in your tenant).

1. In the **Reviewers** field, add **Megan Bowen**, then continue.

1. Review the settings and select **Create policy**, then select **Done**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image14.png)

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image15.png)
...
1. Select **Notice templates**, then select **+** to create a template.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image16.png)

1. On the **Create a notice template** page, complete:

    - **Template name**:

        `Contoso communication notice`

    - **Send from**: select **Megan Bowen**
    - **Cc**: select **Megan Bowen**
    - **Subject**:

        `Your communication may violate Contoso communication compliance policy.`

    - **Message body**:

        `This message was flagged by Contoso's communication compliance policy. Please review company policy and provide a justification for this communication.`


1. Select **Create**.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image17.png)

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image18.png)
...
1. Compose and send an email — to an external address you control or to another internal user — with the subject `Compound results` and a body containing `Sharing pre-publication results for investigational compound CX-2087.` This message contains a Contoso compound identifier and should match the sensitive-information policy.

    ![](https://raw.githubusercontent.com/technofocus-pte/fortifydatasecpurvwdepth/refs/heads/main/Lab%20Guides/Lab-09/media/image19.png)

1. Allow time for the message to be processed by the policy (email can take approximately 24 hours; Teams and Viva Engage up to 48 hours).

1. Sign in to the Microsoft Purview portal as **Megan Bowen** (the reviewer), `MeganB@TenantName`, and select **Solutions** > **Communication Compliance** > **Alerts**.

1. Select the alert generated by the **Detect research and trial data in messages** policy to open it. Review the alert basics — policy, severity, and the matched message — noting that the username may appear anonymized.

1. Open the matched message to examine the full header and body, and review the **Timeline** to see the user's prior policy matches, which provides context for how serious the issue is.

1. Work the message through the remediation actions:
...
1. Confirm the alert moves to the resolved state and that the remediation action is recorded in the user's activity history.
